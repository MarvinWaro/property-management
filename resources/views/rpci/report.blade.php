    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                Report on Physical Count of Inventories - {{ $semester == 1 ? '1st' : '2nd' }} Semester {{ $year }}
                @if($fundCluster)
            ...
            <div class="flex space-x-2">
                ...
                {{-- Export Excel Button --}}
                <a href="{{ route('rpci.export-excel') }}?year={{ $year }}&semester={{ $semester }}&fund_cluster={{ $fundCluster }}&department_id={{ $departmentId ?? '' }}"
                ...
                <div class="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                    ...
                    <form method="GET" action="{{ route('rpci.generate') }}" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <!-- Year Filter -->
                            <div>
                                <label for="filter_year" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Year</label>
                                <select id="filter_year" name="year" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
                            ...
                            <!-- Semester Filter -->
                            <div>
                                <label for="filter_semester" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Semester</label>
                                <select id="filter_semester" name="semester" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
                                    focus:ring-1 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-600
                                    dark:border-gray-500 dark:text-white transition-all duration-200">
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="px-4 py-2 text-white bg-gray-800 rounded-lg hover:bg-gray-900
                        ...
                        <div class="text-gray-600 dark:text-gray-300">
                            ...
                            <div class="flex items-center justify-center space-x-6 text-sm">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    <span>{{ $semester == 1 ? '1st' : '2nd' }} Semester {{ $year }} ({{ $endDate->format('F d, Y') }})</span>
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    @if(!$fundCluster)
                                        <td class="px-6 py-4 text-center">
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium
                                                {{ $item['fund_cluster'] == '101' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300' :
                                                   ($item['fund_cluster'] == '151' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
