<x-app-layout>
    ...
    <!-- REAL-TIME NOTIFICATION SCRIPT FOR STAFF USERS -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Store previous counts to detect changes (get initial values from PHP)
            let previousApprovedCount = {{ $approvedRequestsCount }};
            let previousPendingReceiptCount = {{ $pendingReceiptCount }};
            let isFirstCheck = true;

            // Get badge elements
            const approvedBadge = document.getElementById('user-approved-badge');
            const pendingBadge = document.getElementById('user-pending-badge');

            // Check for user notifications (every 5 seconds for faster updates)
            ...
            const updateUserBadges = (data) => {
                ...
                updateBadge(approvedBadge, data.approved_requests_count, '#6366f1');

                // Update pending receipt badge (with pulse animation)
                updateBadge(pendingBadge, data.pending_receipt_count, '#f59e0b', true);
            };

            // Helper function to update individual badge
            const updateBadge = (badge, count, bgColor, shouldPulse = false) => {
                if (badge) {
                    if (count > 0) {
