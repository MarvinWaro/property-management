        ...
        @if(auth()->user()->hasRole('admin'))
                                        ...
                                        @foreach($risSlip->items as $item)
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 text-sm">
                                                    @php
                                                        $stock = App\Models\SupplyStock::where('supply_id', $item->supply_id)
                                                            ->where('status', 'available')
                                                            ->first();
                                                        $availableQty = $stock ? $stock->quantity_on_hand : 0;
                                                    @endphp

                                                    @if($availableQty >= $item->quantity_requested)
                                                        <span class="px-2 py-1 text-xs rounded-full bg-green-200 text-green-800">{{ $availableQty }}</span>
                                                    @elseif($availableQty > 0)
                                                        <span class="px-2 py-1 text-xs rounded-full bg-yellow-200 text-yellow-800">{{ $availableQty }}</span>
                                                    @else
                                                ...
                                                <td class="px-6 py-4">
                                                    <input type="number" name="items[{{ $loop->index }}][quantity_issued]"
                                                        ...
                                                        min="0" max="{{ min($availableQty, $item->quantity_requested) }}"
                                                        value="{{ min($availableQty, $item->quantity_requested) }}">
                                                </td>
                                                <td class="px-6 py-4">
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (openIssueModal) {
                        openIssueModal.addEventListener('click', function() {
                    ...
                    if (closeIssueModal) closeIssueModal.addEventListener('click', closeModal);
                    if (cancelIssue) cancelIssue.addEventListener('click', closeModal);
                });
            </script>
        @endif
    </x-app-layout>
